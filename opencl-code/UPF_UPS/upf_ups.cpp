#define  CL_TARGET_OPENCL_VERSION 120
   ...
   int main()
   {
       ...
       cl_mem dH   =clCreateBuffer(ctx,CL_MEM_READ_ONLY ,L*sizeof(float),nullptr,&err);
       cl_mem dY   =clCreateBuffer(ctx,CL_MEM_WRITE_ONLY,B*sizeof(float),nullptr,&err);
   
       clEnqueueWriteBuffer(q,dH,CL_TRUE,0,L*sizeof(float),
       ...
       for(unsigned blk=0; blk<=BLKS_IN; blk++)           
       {
           ...
           clEnqueueWriteBuffer(q,dRing,CL_TRUE,0,ringBytes,
           ...
           clEnqueueReadBuffer(q,dY,CL_TRUE,0,copyLen*sizeof(float),
